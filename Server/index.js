...
io.on("connection", (socket) => {
  ...
  //upon connection - to user that connected
  ...
  socket.on("enterRoom", ({ name, room }) => {
    ...
    //can't update prev room users until we update the userState
    if (previousRoom) {
      io.to().emit("userList", {
    ...
    socket.emit(
      "message",
      buildMessage(ADMIN, `You have joined room ${user.room}`)
    );
  ...
  socket.on("disconnect", () => {
    ...
    if (user) {
      io.to(user.room).emit(
        "message",
        buildMessage(ADMIN, `${user.name} has left`)
      );
